...
/**
 * Single source of truth for rendering a delivery address as one line.
 *
 * Three separate copies of this parts-join existed (delivery socket payload,
 * restaurant order notification, and the delivery app), so every new address
 * field had to be added in three places and never was. Ordered outward:
 * flat -> block -> street -> colony -> landmark -> city -> state -> pin.
 */
export function formatDeliveryAddress(address) {
  if (!address || typeof address !== 'object') return '';

  // Trim before prefixing: a whitespace-only flat number is truthy, so
  // prefixing first leaves a bare "Flat" in the address.
  const part = (value) => String(value ?? '').trim();
  const labelled = (label, value) => (part(value) ? `${label} ${part(value)}` : '');

  return [
    labelled('Flat', address.flatNumber),
    labelled('Block', address.blockNumber),
    address.street,
    address.colonyName,
